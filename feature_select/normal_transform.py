...
X_train = pd.DataFrame(normalize(X_train), columns=X_train.columns) 
...
test_id.to_csv("normal_LGBM.csv", index=False)
...
y_pre[y_pre == 'Other'] = 5
test_id['Churn Category'] = y_pre
test_id.to_csv("normal_ADA.csv", index=False)
...
y_pre[y_pre == 'Other'] = 5
test_id['Churn Category'] = y_pre
test_id.to_csv("normal_tree.csv", index=False)