...
X_train = pd.DataFrame(poly.fit_transform(X_train), columns=poly.get_feature_names(X_train.columns))
X_test = pd.DataFrame(poly.fit_transform(X_test), columns=poly.get_feature_names(X_test.columns))
...
test_id.to_csv("poly_LGBM.csv", index=False)
...
y_pre[y_pre == 'Other'] = 5
test_id['Churn Category'] = y_pre
test_id.to_csv("poly_ADA.csv", index=False)
...
y_pre[y_pre == 'Other'] = 5
test_id['Churn Category'] = y_pre
test_id.to_csv("poly_tree.csv", index=False)