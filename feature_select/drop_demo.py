...
df = pd.merge(sat, loca, on="Customer ID", how="outer")
...
df = df.drop(columns=['Count_x', 'Count_y', 'Country', 'State', 'Lat Long'])
...
test_id.to_csv("demo_LGBM.csv", index=False)
...
y_pre[y_pre == 'Other'] = 5
test_id['Churn Category'] = y_pre
test_id.to_csv("demo_ADA.csv", index=False)
...
y_pre[y_pre == 'Other'] = 5
test_id['Churn Category'] = y_pre
test_id.to_csv("demo_tree.csv", index=False)