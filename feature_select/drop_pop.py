...
df = df.drop(columns=['Count_x', 'Count_y', 'Count', 'Country', 'State', 'Lat Long'])
...
test_id.to_csv("pop_LGBM.csv", index=False)
...
y_pre[y_pre == 'Other'] = 5
test_id['Churn Category'] = y_pre
test_id.to_csv("pop_ADA.csv", index=False)
...
y_pre[y_pre == 'Other'] = 5
test_id['Churn Category'] = y_pre
test_id.to_csv("pop_tree.csv", index=False)